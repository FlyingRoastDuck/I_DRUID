...
SAM_CHECKPOINT_PATH = "./pretrained/sam_hq_vit_h.pth"
...
def check_on_image(image = None,prompt = None,gt_bbox = None,attr = None,
                   box_t = 0.25,text_t = 0.25,miou_threshold = 0.5,
                   args = None,image_path = None, img_size=512):
    segment_label = {}
    ...
    detect_mask = torch.zeros(size=(img_size,img_size))
    ...
    color_bg = np.zeros([img_size,img_size,3]).astype(np.uint8) + 127
    image_mid = image_mid + rev_mask * color_bg
    image_mid = image_mid.astype(np.uint8)

    color_mask = check_on_color_cv(image_mid, img_size,prompt,color_dic,attr,args,image_path)
...
def check_on_color_cv(image = None, img_size=512,class_name = None,color_dict = None,color_type = None,args = None,image_path = None):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--file_path',type=str,default='./bench_file/mig_bench.json')
    ...
    if args.need_clip_score:
        clip_model = CLIPModel.from_pretrained('./pretrained/clip/').cuda().eval()
        clip_processor = CLIPProcessor.from_pretrained('./pretrained/clip/')
    ...
    image_path_list = sorted([name for name in os.listdir(image_dir) if name.split('.')[-1].lower() in {'png', 'jpg'}])
    ...
    for image_path in tqdm(image_path_list):
        ...
        '''
        # read gt_label
        image_id = image_path.split('_')[-2].split('.')[0]
        ...
        if need_check_instance:
            if need_segment_instance:
                ...
                for gt_instance in gt_bbox_list:
                    ...
                    gt_bbox = np.array(gt_instance['bbox']) * img_size
                    
                    if args.need_sucess_ratio or args.need_instance_sucess_ratio or args.need_miou_score:
                        sucess_obj, sucess_attr, miou = check_on_image(
                            image, label, gt_bbox,attr,
            ...
            if need_crop_instance:  
                for instance in gt_bbox_list:
                    ...
                    cropped_image = image[int(img_size * inst_bbox[1]):int(img_size * inst_bbox[3]),int(img_size * inst_bbox[0]):int(img_size * inst_bbox[2]),:]
